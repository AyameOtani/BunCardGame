...
public:   // ゲッター・セッター
	...
	// 大きさ設定
	void SetScale(float scale) { mpTexture->SetScale_T(scale); }
	float GetScale() const { return mpTexture->GetScale_T(); }
...
private:
	bool mbIsLoop = false; // ループするかどうか

public:
	void SetLoop(bool loop) { mbIsLoop = loop; }
